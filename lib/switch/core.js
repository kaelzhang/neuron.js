...
Switch = NR.Class({
    ...
    initialize: function(){
        ...
        NR._onceBefore('_runLC', '_initLC', self);
    },
    ...
    _pendingPlugins: function(plugins){
        ...
        NR.provide(pending_plugins, function(){
            var provided = NR.makeArray(arguments),
                _plugins = plugins;
            ...
            pending_indexs.forEach(function(index, i){
                var p = _plugins[index] = provided[i];
    ...
    _addOnePlugin: function(plugin){
        ...
        NR.each(plugin.ATTRS, function(setting, key){
            var undef, value;
     ...
     * @param options {object} NR.Switch options and NR.Switch.Plugins options
     */
