...
return {
    ...
    // @param switchInstance {object} new instance of NR.Switch, as the same as below
    init: function(self){
    	self.on(self.get('EVENTS').BEFORE_INIT, function(){
	        var lazyLoadTextarea,
	            tmp_parent;
	            
	        lazyLoadTextarea = $.findOne(self.CSPre + self.get('lazyLoadCS'));
	        
	        if(lazyLoadTextarea.length){
