...
function absolutize_url(pathname) {
  ...
  assert(base, 'config.path must be specified');
...
function load_by_module(mod) {
  ...
  if (~loaded.indexOf(evidence)) {
    ...
    if (isAsync) {
      pathname = generate_module_pathname(mod);
    } else {
      return;
    }
    ...
    if (combine) {
      var modules = get_all_unloaded_sync_deps(
        mod.name, 
        mod.version, 

        // If is a facade, we should always load it by `mod.id` not `mod.pkg`
        isFacade && removes_leading_slash(mod.path)
      );
      ...
      // `get_all_unloaded_sync_deps` will push loaded.
    } else {
