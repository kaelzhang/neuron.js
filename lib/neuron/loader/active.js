...
function define(name, dependencies, factory){
    ...
    if(arg[last] === true){                    // -> define(uri1, uri2, uri3, true);
        for_each(arg, function(path, i){
            ...
            i < last && _def(EMPTY, UNDEF, UNDEF, applyRelativeURI(path));
        });
        return;
    }
    ...
    if(_define_buffer_on){                  // -> after define.on();
        info = generateModulePathAndURL( moduleNameToURI(name) );
        uri = info.u;
        path = info.p;
        name = EMPTY;
    }else{
...
function _define(name, path, dependencies, factory, uri){
    ...
    // define a module in a module file
        ...
        if(!active_script_uri){
            ...
            mod = getModuleByPath( generateModulePathAndURL(active_script_uri).i );
        }
    }
...
function tidyModuleData(mod){
    if(mod.exports){ console.log('tidyModuleData', KM.clone(mod));
        // free
