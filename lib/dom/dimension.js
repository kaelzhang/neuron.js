...
function getOffsets(element){
    var position = {left: 0, top: 0},
        is_firefox = NR.UA.firefox,
        walk = element,
        parent;

    if (element.getBoundingClientRect && !NR.UA.ios){
    ...
    if (!isBody(element)){
        do {
            ...
            } else if (walk !== element && NR.UA.safari){
                position.left += leftBorder(walk);
                position.top  += topBorder(walk);
            }
...
module.exports = {
    ...
    size: function(element){
        return getSize(getNativeElement(element));
    },
