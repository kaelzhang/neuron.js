...
function encodeObject2Query(obj){
	 ...
	 */
	NR.each(obj, function(value, key){
		var parsed;
	
		if(NR.isPlainObject(value)){
			parsed = [];
		
			// {a:1, b:'2'}		-> 'a=1,b=2'
			// {a:1, b:[]}		-> 'a=1'
			NR.each(value, function(v, k){
				if(NR.isNumber(v) || NR.isString(v)){
					parsed.push(k + '=' + v);
				}
			});
			...
			// [123, '234'] 	-> '123,234'
			// [123, '234', {}] -> '123,234'
			parsed = NR.makeArray(value).map(function(v){
			
					// if the argument is a bool, convert it to 1 or 0	
					return NR.isBoolean(v) ? v ? 1 : 0 : v;
					
				}).filter(function(v){
					return NR.isNumber(v) || NR.isString(v);
...
function decodeQuery2Object(query){
	...
	NR.isString(query) && query.split('&').forEach(function(q){
		q = q.trim();
...
exports.toQuery = encodeObject2Query;
exports.toObject = decodeQuery2Object;
