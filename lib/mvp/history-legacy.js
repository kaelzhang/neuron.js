...
meta = {
	...
	_history: {
		...
		safari: function(){
			// restore the original history.length when NR.Handler initializing
...
handler = {
	...
	_config: function(){
		...
		// options && NR.mix(default_options, options);
		
		// is ie && ( version < 8 || (not IE8 standard mode, ie7 or quirks mode) )
		if(NR.UA.ie && (DOC.documentMode === UNDEF || DOC.documentMode < 8)){
			conf = 'ie';
		
		// webkit 419 or earlier
		// }else if(NR.UA.webkit <= 419){
		//	conf = 'safari';
...
/**
 ...
 2011-04-26  Kael:
 - refractor several methods, NR.history will tidy Number-type arguments
