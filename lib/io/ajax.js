...
_Ajax = NR.Class({
    ...
    initialize: function(options){
        ...
        self.timer = NR.delay(function(){ self.cancel(); }, self.get('timeout'));
    ...
    _stateChange: function(){
        ...
        if (xhr.readyState === DONE && self.running){
            ...
            self.fire( !self.parseError && o.isXHRSuccess(xhr) && o.isSuccess(response) ?
    ...
    send: function(data){
        var self = this,
            ...
            url = String(o.url).replace(REGEX_TRIM_HASH, EMPTY) || NR.getLocation().pathname,
        ...
        NR.each(headers, function(header, key){
