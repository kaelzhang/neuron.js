...
//     }
function provide (dependencies, callback, env){
    var counter = dependencies.length;

    dependencies.forEach(function(dep){
        if (dep) {
            registerModuleLoadCallbackById(dep, function () {
                if(-- counter === 0){
                    callback();
...
// @param {function()} callback
// @param {Object=} env
function registerModuleLoadCallbackById (id, callback, env) {
    var mod = getModuleById(id, env);
    registerModuleLoadCallback(mod, callback);
}
...
// @param {boolean=} main When it is defining a main entry.
function getModuleById (id, env, isMainDefinition){
    var parsed;
