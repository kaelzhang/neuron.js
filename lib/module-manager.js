...
// and dealing with the equivalent modules.
function defineById (id, main) {
  ...
  if (main) {
    ...
    var versions = NEURON_CONF.v(parsed.version);
    equivalents = equivalents.concat(versions).map(function (v) {
      return parsed.n + '@' + v;
    });
  }
...
function registerModuleLoadCallback(mod, callback) {
  ...
  neuron.emit('use', {
    ...
    // @type {boolean=} whether the module is already fetched, i.e. 
    //   we don't need to fetch it from the remote server
    defined: !! mod.factory
  });
}
...
// Create main module of a package
function createPackage (name, version, env) {
  var versions = NEURON_CONF.r();
}
