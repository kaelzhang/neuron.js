...
function parse_id(id, env) {
  ...
  if (relative) {
    env.id || err("Cannot find module '" + id + "'");
...
function generate_exports (module) {
  ...
  var __filename
    // = module.filename 
    = absolutize_url(module_id_to_relative_url_path(module.i));
...
function get_module (id, env, strict) {
  ...
  var real_id = parsed.i;

  // `graph` is the list of modules for a certain package
  var module = graph[real_id];

  if (!module) {
    !strict || err("Cannot find module '" + id + "'");

    // Create module
    var mod = create_mod(parsed);
