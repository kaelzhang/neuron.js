...
function get_module (id, env, strict) {
  ...
  var id = format_package_id(parsed) + parsed.p;
  var deps = env.graph.deps;
  var module = deps[id];

  if (strict) {
    assert(module, "Cannot find module '" + id + "'");
  }

  return module || (
    deps[id] = create_shadow_module(
      create_module_by_id(id)
    )
  );
}
...
function create_require(env) {
  var require = function(id) {
    ...
    var mod = get_module(id, env, true);
