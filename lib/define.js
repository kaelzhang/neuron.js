...
function define(id, dependencies, factory, options) {
  ...
  var mod = modMain || mods[parsed.id] || create_mod(parsed);
  if (options.main) {
    mods[pkg] = mod;
  }
  
  var mod = mods[parsed.i];
  if (!mod) {
    // When define a module,
    // there is no context dependent
    mod = create_mod(parsed);
  }
  ...
  if (!mod.factory) {
    ...
    if (options.main) {
      ...
      if (modMain !== mod) {
        if (modMain) {
          // Combine registered callbacks of `mods['a@1.2.9']` before mixing
          mod.l = mod.l.concat(modMain.l);
          // Clean
          modMain.l.length = 0;
          // Mixing instead of assignment to maintain the prototype chain.
          mix(modMain, mod);

        // This situation, mod is `define()`d before being used.
        } else {
          mods[pkg] = mod;
        }
      }
